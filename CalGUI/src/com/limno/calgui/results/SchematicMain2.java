...
public class SchematicMain2 {
	...
	public class DisplayClickedLabelTask implements Runnable {
		...
		public void run() {
			...
			while (label == null && pel.getParentNode() != null
					&& pel.getParentNode() instanceof Element) {
				...
				if (ptag.equals("g")) {
					...
					for (int i = 0; (label == null)
							&& (i < childNodes.getLength()); i++) {
						Node item = childNodes.item(i);
						if (item instanceof Element) {
							Element ce = (Element) item;
							System.out.println("ce = tag:" + ce.getTagName()
									+ " content: " + ce.getTextContent());
