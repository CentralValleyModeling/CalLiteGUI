...
public class GroupableTableHeaderUI extends BasicTableHeaderUI {
			...
			if (cGroups != null) {
				...
				while (cGroups.hasMoreElements()) {
					...
					if (groupRect == null) {
						...
						Dimension d = cGroup.getSize(header.getTable(), groupHeight);
	...
	private void paintCell(Graphics g, Rectangle cellRect, int columnIndex) {
		...
		Component component = renderer.getTableCellRendererComponent(header.getTable(), aColumn.getHeaderValue(), false, false, -1,
		        columnIndex);
	...
	private void paintCell(Graphics g, Rectangle cellRect, ColumnGroup cGroup) {
		...
		TableCellRenderer renderer = header.getTable().getTableHeader().getDefaultRenderer();
		Component component = renderer.getTableCellRendererComponent(header.getTable(), cGroup.getHeaderValue(), false, false, -1,
		        -1);
		rendererPane.add(component);
		rendererPane.paintComponent(g, component, header, cellRect.x, cellRect.y, cellRect.width, cellRect.height, true);
	}
		...
		for (int column = 0; column < columnModel.getColumnCount(); column++) {
			...
			if (enumeration != null) {
				while (enumeration.hasMoreElements()) {
					...
					cHeight += cGroup.getSize(header.getTable(), cHeight).height;
				}
			}
