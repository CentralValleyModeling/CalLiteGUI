...
/**
 * 
 * Variant of BoxPlotChartPanel specifically for plotting WRIMS GUI MultipleTimeSeriesobjects
 * 
 * @author tslawecki
 * 
 */
public class BoxPlotChartPanel2 extends JPanel implements Printable {
	...
	public BoxPlotChartPanel2(String title, String yLabel, TimeSeriesContainer[][] mtscs, TimeSeriesContainer[][] mstscs,
	        Date lower, Date upper, String sLabel, boolean isBase, MultipleTimeSeries mts) {
		...
		for (int mtsI = 0; mtsI < mtscs.length; mtsI++) {
			...
			for (int i = 0; i < seriesCount; i++) {
				for (int j = 0; j < categoryCount; j++) {
					...
					dataset.add(list, tscs[i].fileName, j == 0 ? "All" : j == 13 ? "Annual"
