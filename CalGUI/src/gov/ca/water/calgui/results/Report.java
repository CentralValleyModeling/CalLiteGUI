...
public class Report extends SwingWorker<Void, String> {
	...
	public void doProcessing() {
		...
		for (PathnameMap pathMap : pathnameMaps) {
			...
			DataReference refBase = Utils.getReference(dssGroupBase, pathMap.pathBase, calculate_dts, pathnameMaps, 1);
			DataReference refAlt = Utils.getReference(dssGroupAlt, pathMap.pathAlt, calculate_dts, pathnameMaps, 2);
			...
			if (pathMap.plot) {
				if (pathMap.report_type.startsWith("average")) {
					generatePlot(Utils.buildDataArray(refAlt, refBase, tw), dataIndex,
					...
					generatePlot(Utils.buildExceedanceArray(refAlt, refBase, pathMap.var_category == "S_SEPT", tw), dataIndex,
					...
					generatePlot(Utils.buildExceedanceArray(refAlt, refBase, true, tw), dataIndex,
					        "Exceedance " + pathMap.var_name.replace("\"", ""), series_name, "Allocation (%)", "Probability",
					        PlotType.EXCEEDANCE);
				}
			}
		}
	...
	private void generateSummaryTable() {
		...
		for (PathnameMap pathMap : pathnameMaps) {
			...
			if (!pathMap.pathBase.equalsIgnoreCase("ignore")) {
				refBase = Utils.getReference(dssGroupBase, pathMap.pathBase, calculate_dts, pathnameMaps, 1);
			}
			if (!pathMap.pathAlt.equalsIgnoreCase("ignore")) {
				refAlt = Utils.getReference(dssGroupAlt, pathMap.pathAlt, calculate_dts, pathnameMaps, 2);
			}
			for (TimeWindow tw : timewindows) {
				double avgBase = 0, avgAlt = 0;
				if (refAlt != null) {
					avgAlt = Utils.avg(Utils.cfs2taf((RegularTimeSeries) refAlt.getData()), tw);
					rowData.add(formatDoubleValue(avgAlt));
				} else {
					rowData.add("");
				}
				if (refBase != null) {
					avgBase = Utils.avg(Utils.cfs2taf((RegularTimeSeries) refBase.getData()), tw);
